...
class Controller {
    ...
    public function verificarProductoCodigo($codigo) {
        try {
            $stmt = $this->conn->prepare("SELECT COUNT(*) FROM producto WHERE producto_codigo = :codigo");
    ...
    public function registrarProducto($datos) {
        if ($this->verificarProductoCodigo($datos['producto_codigo'])) {
            return json_encode(['status' => 'error', 'message' => 'Ya existe un producto con el mismo código']);
        }
