...
class loginController extends mainModel {
        ...
        if ($usuario == "" || $clave == "") {
            ...
            if ($this->verificarDatos("[a-zA-Z0-9]{4,20}", $usuario)) {
                ...
                if ($this->verificarDatos("[a-zA-Z0-9$@.-]{7,100}", $clave)) {
                    ...
                    $check_usuario = $this->ejecutarConsulta($query, $params);
    ...
    public function seleccionarDatos($tipo, $tabla, $campo, $valor) {
        if ($tipo === "Unico") {
            $query = "SELECT * FROM $tabla WHERE $campo = :valor LIMIT 1";
            $params = [":valor" => $valor];
            $stmt = $this->ejecutarConsulta($query, $params);
            return $stmt; // Devuelve el resultado
        }
        return null; // Maneja otros tipos según sea necesario
    }
}